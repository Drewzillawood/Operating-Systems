...
void *producer(void *arg)
{
  ...
  while (!producer_done)
  {
    ...
    /* Wait until supply dwindles to zero */
    while(supply > 0 && num_cons_remaining > 0)
      pthread_cond_wait(&producer_cv, &mut);

    // Once our supplies is zero, and we still have
    // threads left, produce 10 supplies
    if(supply == 0 && num_cons_remaining > 0)
    {
      supply += 10;
    ...
    {
      // Once there are no more consumeres, terminate
      producer_done++;
    }
...
void *consumer(void *arg)
{
  ...
  printf("consumer thread id %d consumes an item... %d\n", cid, supply);
  fflush(stdin);
