...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  int current_page = PageAccesses[current_access];
  int count = 0;
  int flag = 0;
  int max = 0;

  // Look at each page frame
  for(int i = 0; i < num_frames; i++)
  {
    // At each access ahead of the current access
    for(int j = current_access; j < num_accesses; j++)
    {
      // If the page frame ID and access ID are the same it will be used "soon"
      if(PageFrames[i].page_id == PageAccesses[j])
      {
        // Measure the time between current access and next used
        count = j - current_access;
        break;
      }
    }
    // Keep track of max time until used
    if(count > max)
    {
      max = count;
      flag = i;
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  int accessed_least_recently = PageFrames[0].time_of_access;
  int index_with_least_recently_used = 0;

  // Search through num_frames and just find which one was least recently used
  for(int i = 0; i < num_frames; i++)
  {
    if(PageFrames[i].time_of_access < accessed_least_recently)
    {
      index_with_least_recently_used = i;
    }
  }

  return index_with_least_recently_used;
}
...
int PRAlgo_CUST(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  // We know the (LR) Input has a chance of being among the 128 pages instead of local, thus we will use 
  // the benefits of OPT along with LRU to achieve the minimal number of page faults
  int current_page = PageAccesses[current_access];
  int count = 0;
  int flag = 0;
  int max = 0;
  ...
  return (current_access - index_with_least_recently_used) > max ? index_with_least_recently_used : flag;
}